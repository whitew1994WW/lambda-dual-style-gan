...
class TestOptions:
    def __init__(self):
        ...
        self.parser = argparse.ArgumentParser(
            description="Random Artistic Portrait Generation"
        )
        self.parser.add_argument(
            "--batch", type=int, default=8, help="number of generated images"
        )
        self.parser.add_argument(
            "--style", type=str, default="cartoon", help="target style type"
        )
        self.parser.add_argument(
            "--truncation",
            type=float,
        ...
        self.parser.add_argument(
            "--fix_color",
            action="store_true",
            help="using a fixed extrinsic color code (style)",
        )
        self.parser.add_argument(
            "--fix_structure",
            action="store_true",
...
if __name__ == "__main__":
    ...
    ckpt = torch.load(
        os.path.join(args.model_path, args.style, args.sampler_name),
        map_location=lambda storage, loc: storage,
    )
    ...
    with torch.no_grad():
        ...
        res_in = icpts.icp.netT(torch.randn(args.batch, 128).to(device)).reshape(
            -1, 7, 512
        )
        # sample color codes
        ada_in = icptc.icp.netT(torch.randn(args.batch, 128).to(device)).reshape(
            -1, 11, 512
        )
    ...
    for i in range(args.batch):
        save_image(
            img_gen[i].cpu(),
            os.path.join(args.output_path, args.name + "_%02d.jpg" % (i)),
        )
