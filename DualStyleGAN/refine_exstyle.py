...
class TrainOptions:
    def __init__(self):
        ...
        self.parser = argparse.ArgumentParser(
            description="Refine Extrinsic Style Codes"
        )
        ...
        self.parser.add_argument(
            ...
            type=str,
            default=None,
            help="path to the saved dualstylegan model",
        )
        self.parser.add_argument(
            "--exstyle_path",
            type=str,
        ...
        self.parser.add_argument(
            "--lr_color", type=float, default=0.01, help="learning rate for color parts"
        )
        self.parser.add_argument(
            "--lr_structure",
            type=float,
            default=0.005,
            help="learning rate for structure parts",
        )
        self.parser.add_argument(
            "--model_name",
            type=str,
            default="refined_exstyle_code.npy",
    ...
    def parse(self):
        ...
        if self.opt.ckpt is None:
            self.opt.ckpt = os.path.join(
                self.opt.model_path, self.opt.style, "generator.pt"
            )
...
if __name__ == "__main__":
    ...
    for ii in range(0, len(files), args.batch):
        ...
        optimizer = optim.Adam(
            [
                {"params": exstyles_c, "lr": args.lr_color},
        ...
        for i in pbar:
            ...
            img_gen, _ = generator(
                [instyles], latent, noise=noises, use_res=True, z_plus_latent=True
            )
        ...
        with torch.no_grad():
            ...
            for j in range(imgs.shape[0]):
                ...
                save_image(
                    torch.clamp(vis.cpu(), -1, 1),
                    os.path.join(
                        "./log/%s/refine_exstyle/" % (args.style), batchfiles[j]
                    ),
                )
