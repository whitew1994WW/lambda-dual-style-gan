...
class TrainOptions:
    def __init__(self):
        ...
        self.parser.add_argument(
            "--start_iter", type=int, default=0, help="start iteration"
        )
        self.parser.add_argument(
            "--save_every",
            type=int,
        ...
        self.parser.add_argument(
            ...
            type=int,
            default=1000,
            help="when to start saving a checkpoint",
        )
        self.parser.add_argument(
            "--subspace_freq", type=int, default=2, help="how often to use paired data"
        )
        self.parser.add_argument(
            "--style_loss",
            type=float,
            default=0.25,
            help="the weight of feature matching loss",
        )
        self.parser.add_argument(
            "--CX_loss", type=float, default=0.25, help="the weight of contextual loss"
        )
        self.parser.add_argument(
            "--perc_loss", type=float, default=1.0, help="the weight of perceptual loss"
        )
        self.parser.add_argument(
            "--id_loss", type=float, default=1.0, help="the weight of identity loss"
        )
        self.parser.add_argument(
            "--L2_reg_loss",
            type=float,
            default=0.01,
            help="the weight of identity loss",
        )
        self.parser.add_argument(
            "--model_path",
            type=str,
        ...
        self.parser.add_argument(
            "--encoder_path", type=str, default=None, help="path to the encoder model"
        )
        self.parser.add_argument(
            "--image_path", type=str, default=None, help="path to the image dataset"
        )
        self.parser.add_argument(
            "--identity_path", type=str, default=None, help="path to the identity model"
        )
        self.parser.add_argument(
            "--lmdb_path", type=str, default=None, help="path to the lmdb dataset"
        )
        self.parser.add_argument(
            "--exstyle_path",
            type=str,
            default=None,
            help="path to the extrinsic style codes",
        )
        self.parser.add_argument(
            "--instyle_path",
            type=str,
            default=None,
            help="path to the intrinsic style codes",
        )
        self.parser.add_argument(
            "--model_name", type=str, default="generator", help="name of saved model"
        )
    ...
    def parse(self):
        ...
        if self.opt.identity_path is None:
            self.opt.identity_path = os.path.join(
                self.opt.model_path, "model_ir_se50.pth"
            )
        if self.opt.image_path is None:
            self.opt.image_path = os.path.join(
                "./data", self.opt.style, "images/train/"
            )
        ...
        if self.opt.exstyle_path is None:
            self.opt.exstyle_path = os.path.join(
                self.opt.model_path, self.opt.style, "exstyle_code.npy"
            )
        if self.opt.instyle_path is None:
            self.opt.instyle_path = os.path.join(
                self.opt.model_path, self.opt.style, "instyle_code.npy"
            )
        args = vars(self.opt)
...
def get_paired_data(
    instyles, Simgs, exstyles, subspace_std=0.1, batch_size=4, random_ind=8
):
...
def train(
    ...
    sample_exstyle, _, _ = get_paired_data(
        instyles, Simgs, exstyles, batch_size=args.n_sample, random_ind=8
    )
    ...
    for idx in pbar:
        ...
        if which == 0:
            ...
            exstyle, _, _ = get_paired_data(
                instyles, Simgs, exstyles, batch_size=args.batch, random_ind=8
            )
            ...
            exstyle, instyle, real_img = get_paired_data(
                instyles, Simgs, exstyles, batch_size=args.batch, random_ind=8
            )
            exstyle = exstyle.to(device)
        ...
        fake_img, _ = generator(
            instyle, exstyle, use_res=True, z_plus_latent=z_plus_latent
        )
        ...
        if which == 0:
            # sample z^+_e, z for Lsty, Lcon and Ladv
            exstyle, _, real_img = get_paired_data(
                instyles, Simgs, exstyles, batch_size=args.batch, random_ind=8
            )
            real_img = real_img.to(device)
            exstyle = exstyle.to(device)
        ...
        with torch.no_grad():
            ...
            real_content, _ = generator(
                instyle, None, use_res=False, z_plus_latent=z_plus_latent
            )
        ...
        fake_styles = [
            F.adaptive_avg_pool2d(fake_feat, output_size=1) for fake_feat in fake_feats
        ]
        sty_loss = (
            torch.tensor(0.0).to(device)
            if args.CX_loss == 0
            else FCX.contextual_loss(
                fake_feats[2],
                real_feats[2].detach(),
                band_width=0.2,
                loss_type="cosine",
        ...
        if args.style_loss > 0:
            sty_loss += (
                F.mse_loss(fake_styles[1], real_styles[1])
        ...
        ID_loss = (
            torch.tensor(0.0).to(device)
            if args.id_loss == 0
            else id_loss(fake_img_256, real_content_256) * args.id_loss
        )
        ...
        if which > 0:
            for ii, weight in enumerate(vgg_weights):
                if weight * args.perc_loss > 0:
                    gr_loss += (
                        F.l1_loss(fake_feats[ii], real_feats[ii].detach())
                        * weight
                        * args.perc_loss
                    )
        ...
        l2_reg_loss = (
            sum(torch.norm(p) for p in g_module.res.parameters()) * args.L2_reg_loss
        )
        ...
        if g_regularize:
            ...
            exstyle, _, _ = get_paired_data(
                instyles, Simgs, exstyles, batch_size=path_batch_size, random_ind=8
            )
            exstyle = exstyle.to(device)

            fake_img, latents = generator(
                instyle, exstyle, return_latents=True, use_res=True, z_plus_latent=False
            )
        ...
        if get_rank() == 0:
            ...
            if ((i + 1) >= args.save_begin and (i + 1) % args.save_every == 0) or (
                i + 1
            ) == args.iter:
                torch.save(
                    {
...
if __name__ == "__main__":
    ...
    for filename in instyle_dict.keys():
        ...
        with torch.no_grad():
            ...
            exstyle = (
                g_ema.generator.style(exstyle[0]).reshape(exstyle.shape).detach().cpu()
            )
    ...
    instyles = torch.tensor(
        np.concatenate(instyles, axis=0)
    )  # instrinsic style codes z^+_i
    Simgs = torch.cat(Simgs, dim=0)  # image S
    exstyles = torch.tensor(
        np.concatenate(exstyles, axis=0)
    )  # exstrinsic style codes z^+_e
