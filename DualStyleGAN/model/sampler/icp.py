...
OptParams = collections.namedtuple(
    "OptParams", "lr batch_size epochs " + "decay_epochs decay_rate "
)
...
class _ICP:
    ...
    def train_epoch(self, z_np, epoch):
        ...
        optimizerT = optim.Adam(
            self.netT.parameters(), lr=lr, betas=(0.5, 0.999), weight_decay=1e-5
        )
...
class ICPTrainer:
    ...
    def train_icp(self, n_epochs):
        uncca_opt_params = OptParams(
            lr=1e-3, batch_size=128, epochs=n_epochs, decay_epochs=50, decay_rate=0.5
        )
