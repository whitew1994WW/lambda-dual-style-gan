...
class TestOptions:
    def __init__(self):
        ...
        self.parser.add_argument(
            "--iter", type=int, default=300, help="total training iterations"
        )
...
if __name__ == "__main__":
    ...
    id_loss = (
        id_loss.IDLoss(os.path.join(args.model_path, "model_ir_se50.pth"))
        .to(device)
        .eval()
    )
    ...
    for ii in range(0, len(files), args.batch):
        ...
        with torch.no_grad():
            ...
            img_rec, latent_e = encoder(
                imgs, randomize_noise=False, return_latents=True, z_plus_latent=True
            )
        ...
        for i in pbar:
            ...
            # g'(z^+)
            img_gen, _ = generator_prime(
                [latent_n], input_is_latent=False, noise=noises, z_plus_latent=True
            )
        ...
        with torch.no_grad():
            ...
            # (optinal) preserve color
            _, latent_i = encoder(
                img_dsty, randomize_noise=False, return_latents=True, z_plus_latent=True
            )
            # z^+_i
            latent_i[:, 8:18] = latent_e[:, 8:18].detach()
            # g(hat(z)^+_i)
            img_refine, _ = generator(
                [latent_i.detach()],
                input_is_latent=False,
                truncation=args.truncation,
                truncation_latent=0,
            ...
            for j in range(imgs.shape[0]):
                vis = torchvision.utils.make_grid(
                    torch.cat(
                        [
                            imgs[j : j + 1],
                            img_rec[j : j + 1].detach(),
                            img_dsty[j : j + 1].detach(),
                            img_refine[j : j + 1].detach(),
                        ],
                        dim=0,
                    ),
                    4,
                ...
                save_image(
                    ...
                    os.path.join(
                        "./log/%s/destylization/" % (args.style), batchfiles[j]
                    ),
                )
