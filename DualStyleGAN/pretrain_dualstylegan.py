...
class TrainOptions:
    def __init__(self):
        ...
        self.parser.add_argument(
            "--iter", type=int, default=3000, help="total training iterations"
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            ...
            type=int,
            default=4,
            help="how often to use Gaussian style code",
        )
        self.parser.add_argument(
            "--model_name",
            type=str,
            default="generator-pretrain",
    ...
    def parse(self):
        ...
        if self.opt.ckpt is None:
            self.opt.ckpt = os.path.join(
                self.opt.model_path, "stylegan2-ffhq-config-f.pt"
            )
        args = vars(self.opt)
...
def pretrain(
    ...
    for idx in pbar:
        ...
        fake_img, _ = generator(
            noise, externalstyle, use_res=True, z_plus_latent=z_plus_latent
        )
        ...
        if get_rank() == 0:
            ...
            if i % 300 == 0 or (i + 1) == args.iter:
                with torch.no_grad():
                    g_ema.eval()
                    sample, _ = g_ema(
                        [sample_zs[0].unsqueeze(1).repeat(1, g_module.n_latent, 1)],
                        sample_style,
                        use_res=True,
                        z_plus_latent=True,
                    )
